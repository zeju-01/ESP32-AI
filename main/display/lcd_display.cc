...
void LcdDisplay::SetupUI() {
    ...
    if (use_robot_face_) {
        int face_size = std::min(width_, height_) * 2 / 2.4;//std::min(width_, height_)：取屏幕宽度和高度中的最小值
                                                            //* 2 / 3：将最小值乘以 2/3，作为表情容器的大小
                                                          //示例表情容器大小计算
                                                            //如果屏幕是 240x135，容器大小 = 135 * 2/3 = 90
                                                            //如果屏幕是 240x240，容器大小 = 240 * 2/3 = 160
                                                            //如果屏幕是 320x240，容器大小 = 240 * 2/3 = 160
        robot_face_ = std::make_unique<RobotFace>(emoji_box_, face_size);
        lv_obj_add_flag(robot_face_->GetCanvas(), LV_OBJ_FLAG_HIDDEN);
        ESP_LOGI(TAG, "RobotFace created with size %d", face_size);
    }
...
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    if (use_robot_face_ && robot_face_) {
        ...
        lv_obj_add_flag(emoji_image_, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(emoji_label_, LV_OBJ_FLAG_HIDDEN);
        lv_obj_remove_flag(robot_face_->GetCanvas(), LV_OBJ_FLAG_HIDDEN);
        ESP_LOGD(TAG, "Using RobotFace for emotion: %s", emotion);
        return;
    }
